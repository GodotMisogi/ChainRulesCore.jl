...
#### Less formally
 ...
 - The **pullback** takes wobbliness information with respect to the function's output, and tells the equivalent wiggliness with respect to the function's input.
...
### Math
This is simplified by talking in 1D.
...
#### Geometric interpretation of reverse and forward mode AD
...
``g^*(dh)(X) = g_*(X)(dh) = X(g\circ h)`` for any function ``h`` on ``N`` and ``X\in TM``. In particular,
``g^*(dy_j)(d/dx_i) = d(g_j)/dx_i``. If you work out the action on a basis of the cotangent space, you see that it acts
by the adjoint of the Jacobian.

Notice that the pullback of a differential and the pushforward of a vector have very different meanings, and this should
...
This works in a coordinate-invariant way without the notion of a metric.
_Gradients_ recall are vectors, yet they should contain the same information as the differential ``df``.
Using the standard Euclidean metric, we can identify ``df`` and ``\nabla f`` as vectors.
...
Considered by itself, it would look like:
...
It is common to write `function foo_pushforward(_, Δargs...)` when `foo` does not have fields.
...
- **Pullback**
   ...
   - Argument structure matches the structure of the primal function output
   - If the primal function returns a tuple, then pullback takes in a tuple of differentials.
   - 1 return per original function argument + 1 for the function itself

- **Pushforward:**
    - part of `frule`
    - takes input space wiggles, gives output space wobbles
    - Argument structure matches primal function argument structure, but passed as a tuple at the start of `frule`
